...
更重要的是，\textbf{这个近似是良性的}。在代价尚可承受的秩 $(8,6,10)$ 上直接比较两者，
精度几乎相同，估出的超参也在同一量级：
...
目标 & NRMSE & $\alpha$ & $\beta$\\
\midrule
当前方案（因子 MAP $+$ 核精确后验） & $0.0307$ & $4.8\times10^{2}$ & $9.33\times10^{1}$\\
边缘似然（核积掉，对因子求极大） & $0.0300$ & $6.3\times10^{2}$ & $9.40\times10^{1}$\\
\bottomrule
\end{tabular}
\end{center}

所以这个分工不是"只能这样做"，而是\textbf{精确做也是同一个结果，但贵 $18$ 倍}：
它把 $O(nP^2)$ 的那一步从每步一次降到全程一次，使方法在能真正兑现基的秩上仍然可算。
