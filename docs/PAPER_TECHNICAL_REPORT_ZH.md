...
布局按"几何有多重要"排序，用几何盲基的投影残差度量：

| 布局 | 几何盲偏差下限 | 几何感知偏差下限 | 比值 |
|---|---:|---:|---:|
| `open`（无障碍，阴性对照） | 0.0034 | 0.0034 | 1.0× |
| `labyrinth` | 0.0395 | 0.0035 | 11× |
| `arc`（弧形，坐标网络难以表示） | 0.0643 | 0.0046 | 14× |
| `chamber` | 0.1684 | 0.0031 | 55× |
| `sealed_4`（四个密封象限） | 0.2460 | 0.0024 | 101× |
...
搬到几何 B，只把 `Φ_A` 换成 `Φ_B`。**few-shot**（目标域仅 2% 传感器，且只重拟合小
core）是诚实的主张形式；**zero-shot 我们较差**，因为跨几何的模态序号对应没有校准，
这必须作为局限如实报告。
